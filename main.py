import player, enemy, inventory_manager as im, abilities_manager as am, UI_manager as UI, text_pop as tp, random, fight, shop, team, battle_result, skills_window, character_creation
...
class Menu(Screen):
    def __init__(self, **kw):
        super().__init__(**kw)
        self.stage_progress = StageProgressBar(max = 10)
        self.stage_text = Label(font_size = 25, pos=(635,0))
        self.add_widget(self.stage_progress)
        self.add_widget(self.stage_text)
        self.setup_window()
        print(player.main_player.skill)
    def setup_window(self):
        self.stage_progress.value = fight.current_fight
        self.stage_text.text = str(int(self.stage_progress.value))+" / 10"
